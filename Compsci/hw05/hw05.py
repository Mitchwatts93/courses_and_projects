...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        def interest_loop(new_balance,interest):
            if new_balance>=amount:
                return 0
            else:
                return 1 + interest_loop(new_balance * interest,interest)
        return interest_loop(self.balance, 1+self.interest)
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        """Subtract amount from balance if funds are available."""
        charge = self.free_withdrawals < 1
        self.free_withdrawals = self.free_withdrawals - 1
        if (amount+self.withdraw_fee * charge) > self.balance:
            return 'Insufficient funds'
        self.balance = self.balance - amount - self.withdraw_fee * charge
...
def make_withdraw(balance, password):
    ...
    attempts = [0,0,0]
    attcount = 0
    def withdrawl_password(amount, password):
        nonlocal locked, passcode, balance, attcount
        if password == passcode and locked == False:
            if amount> balance:
                return 'Insufficient funds'
            balance = balance - amount
            message = balance
        elif locked == True:
            message = "Your account is locked. Attempts: "+str(attempts)
        elif password != passcode:
            attempts[attcount] = password
            attcount += 1
            if attcount == 3:
                locked = True
                #return withdrawl_password(amount, password)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert not (lower_bound(y) <= 0 <= upper_bound(y)), "don't divide by zero"
