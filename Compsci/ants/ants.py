...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return not other.container and (self.container) and (not self.ant)
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bee_place = self.place  # this gives you the current place to look in
        counter = 0
        while bee_place != hive:
            if random_or_none(bee_place.bees) and (counter <= self.max_range or counter >= self.min_range):
                return random_or_none(bee_place.bees)
            else:
                bee_place = bee_place.entrance # this gives you the place which is the entrance to current place
                counter += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            bees_copy = self.place.bees[:] # gives the bees at the current location
            self.place.remove_insect(self) # removes the ant from the board     
            for i in bees_copy: #iterate over bees at current location
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for i in bees_copy: #iterate over bees
            i.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    def __init__(self, digesting=0, armor=1):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        bees_copy = self.place.bees[:]  
        for i in bees_copy: #iterate over
            i.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true:
            amount = self.armor
            Insect.reduce_armor(self, amount)  # imposter queens get all armor removed
        else:
            ThrowerAnt.action(self, colony) #normal thrower ant action

            # double damage of all ants behind
            location = self.place.exit
            while location!=None:
                to_be_doubled = location.ant  # ant at location
                if to_be_doubled!=None and to_be_doubled not in self.doubled_damage:
                    to_be_doubled.damage = to_be_doubled.damage * 2
                    self.doubled_damage.append(to_be_doubled)
                if to_be_doubled!=None and to_be_doubled.container: #if its a container ant
                    if to_be_doubled.ant not in self.doubled_damage and to_be_doubled.ant != None:
                        to_be_doubled.ant.damage = to_be_doubled.ant.damage * 2
                        self.doubled_damage.append(to_be_doubled.ant)
...
def apply_effect(effect, bee, duration):
    ...
    old_action = bee.action # remember the old effect
    altered_action = effect(bee.action) # binds the effect to the bees action

    def new_action(colony):
        nonlocal duration
        if duration > 0:
            altered_action(colony)
        else:
            old_action(colony)
        duration -= 1
